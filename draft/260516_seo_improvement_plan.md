...
- 下層ページのtitle/h1のキーワード強化（施策6）
- 画像サイトマップ追加・JSON-LD整備・残ページのog:image個別化（施策8）
- トップページの信頼性シグナル追加（撮影実績数、地域名密度）（施策1）
...
#### A2. 画像サイトマップとJSON-LDによるマルチイメージSERP対応（施策8）
...
#### A3. トップh1/h2の地域名密度強化（施策1）

現状トップh1は「little seeds photography」のみで、サブタイトルに「藤沢市を拠点に湘南エリア・神奈川・東京で」と記載。
**藤沢・茅ヶ崎・鎌倉の3市明記**にリライト。

- hero subtitle: 「藤沢・茅ヶ崎・鎌倉を中心に湘南エリア・神奈川・東京で<br>ニューボーンフォトの出張撮影をしています」
- ファーストビュー直下に「対応エリア: 藤沢・茅ヶ崎・鎌倉を中心に」のテキスト追加（visually-hiddenではなく可視テキスト）
